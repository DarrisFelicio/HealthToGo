...
class _SleepScreenState extends State<SleepScreen> {
  ...
  void _saveSleepData() async {
    ...
    try {
      ...
      await firebaseService.addSleepData(sleepData);
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                const Icon(Icons.bedtime, color: Color(0xFF9C27B0)),
  ...
  Future<void> _deleteSleepData(String id) async {
    ...
    if (confirmed == true) {
      try {
        await Provider.of<FirebaseService>(context, listen: false).deleteSleepData(id);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
